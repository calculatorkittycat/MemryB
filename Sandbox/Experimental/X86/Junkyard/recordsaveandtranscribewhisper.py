...
# Directory constants
LAMA_DIR = r"C:\codes\llama32\Llama-3.2-3B-Instruct"
TRANSCRIPT_DIR = r"Sandbox/Experimental/X86/Output"
RECORDING_DIR = r"C:\Users\cicai\PycharmProjects\memoryBeta\Recordings"
LAMA_TRANSCRIPT_DIR = os.path.join(LAMA_DIR, "Transcripts")  # Directory for duplicate transcripts
FLAG_FILENAME = os.path.join(LAMA_TRANSCRIPT_DIR, "transcription_name.flag")  # Path for the flag file
...
def main():
    ...
    os.makedirs(LAMA_TRANSCRIPT_DIR, exist_ok=True)  # Ensure LAMA transcript directory exists
    ...
    lama_text_output_filename = os.path.join(LAMA_TRANSCRIPT_DIR,
                                             generate_filenames("transcription") + ".txt")  # Duplicate path
    ...
    # Save duplicate transcription to the LAMA directory
    with open(lama_text_output_filename, 'w') as lama_text_file:
        lama_text_file.write(transcription_result)
    ...
    # Save the flag file with the transcription name
    with open(FLAG_FILENAME, 'w') as flag_file:
        flag_file.write(os.path.basename(lama_text_output_filename))  # Save the name of the transcription file

    print(f"Flag file saved to: {FLAG_FILENAME}")
